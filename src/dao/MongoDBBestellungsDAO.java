...
public class MongoDBBestellungsDAO implements BestellungsDAO {
	...
	public boolean createBestellungFromWarenkorb(Bestellung bestellung, String date) {
		...
		if(this.getBestellungByID(id)!=null){
			System.out.print("MongoDB: Bestellung mit dem id existiert schon");
			return false;
		}
		try{

			Document neueBestellung = new Document().append("_id", id)
					.append("gesamtpreis", gesamtpreis)
					.append("abgeschlossen", abgeschlossen)
					.append("lieferant", liferart)
					.append("vermerk",vermerkt)
					.append("datum", datum);
	...
	public boolean createPosition(int bestellungsID, Position position) {
		...
		if(this.getPositionByID(bestellungsID, pid)!=null){
			System.out.print("MongoDB: position mit dem id existiert schon");
			return false;
		}
		try{
			Document createPosition = new Document().append("_id", id)
					.append("positionId",pid)
					.append("menge", menge)
					.append("preis",preis)
	...
	public boolean createWarenkorb(int kundenID) {
		int kid = kundenID;
		try{
			Document createWarenkorb = new Document()
					.append("positionId",kid);
	...
	public Bestellung getBestellungByID(int bestellungsID) {
		try {
			FindIterable<Document> documents = db.getCollection(collectionName)
					.find(new Document("id", bestellungsID));
			...
			for(Document d1 : documents){
				if(d1.get("adminDaten") != null){
					...
					bestellung = new Bestellung(bestellungsID1,
							gesamtpreis,
							abgeschlossen,
							datum,
							vermerk,
	...
	public Position getPositionByID(int bestellungsID, int positionID) {
		...
		try {
			FindIterable<Document> documents = db.getCollection(collectionName)
					.find(new Document("id", bestellungsID));
			FindIterable<Document> documents1 = db.getCollection(collectionName)
					.find(new Document("pid", positionID));
			...
			for(Document d : documents1){
				if(d.get("adminDaten") != null){
					position = new Position(
							d.getInteger("positionID"), 
							d.getInteger("produktID"), 
							d.getInteger("menge"), 
							d.getDouble("gesamtpreis"));
				}
			}
			return position;	
		...
		try{
			for(Document d : documents){
				if(d.get("neuePosition") != null){
					positionListe.add( new Position(
							d.getInteger("positionID"), 
							d.getInteger("produktID"), 
							d.getInteger("menge"), 
							d.getDouble("gesamtpreis")));
	...
	public boolean removePosition(int bestellungsID, int positionID) {
		...
		try{
			Document createPosition = new Document().append("_id", bestellungsID)
